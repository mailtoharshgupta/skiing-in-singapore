...
public class SkiingInSingapore {
     ...
     * 1. It takes the path of the file and parses it into a matrix.
     * 2. Applies DFS to it's cells and stores the longest path and largest drop.
     */
        ...
        for (int i = 0; i < 4; i++) {
            if (isGridAllowed(x + util[0][i], y + util[1][i], mountainMatrix, mountainMatrix.length, mountainMatrix[x].length, mountainMatrix[x][y])) {
                ...
                if (tempSkiingPath.size() > currentSkiingPath.size()) {
                    ...
                    // This step is to choose the gridlist with larger drop
                    currentSkiingPath =
     ...
     * @return
     * @throws IOException
     * @throws URISyntaxException A util function to parse data from the file into a matrix
     */
